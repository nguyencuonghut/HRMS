...
import otherBusinessCatalogService, { type ReligionRead } from '@/services/otherBusinessCatalogService'
...
const selected = ref<ReligionRead | null>(null)
const suggestions = ref<ReligionRead[]>([])
...
async function hydrate() {
  ...
  loading.value = true
  try {
    const res = await otherBusinessCatalogService.getReligionById(props.modelValue)
...
async function search(event: { query: string }) {
  loading.value = true
  try {
    const res = await otherBusinessCatalogService.lookupReligions({ keyword: event.query.trim() || undefined, limit: 20 })
