...
var script$3 = {
  ...
  methods: {
    ...
    onKeyDown: function onKeyDown(event2) {
      if ((event2.code === "Enter" || event2.code === "NumpadEnter" || event2.code === "Space") && event2.currentTarget.nodeName === "TH" && Q(event2.currentTarget, "data-p-sortable-column")) {
        this.$emit("column-click", {
          originalEvent: event2,
          column: this.column
        });
...
var script$1 = {
  ...
  methods: {
    ...
    onClick: function onClick2(event2) {
      if (Dt(event2.target) || Q(event2.target, "data-pc-section") === "nodetogglebutton" || Q(event2.target, "data-pc-section") === "nodetoggleicon" || event2.target.tagName === "path") {
        return;
      }
    ...
    onEndKey: function onEndKey(event2) {
      var nodes = Y(event2.currentTarget.parentElement, 'tr[aria-level="'.concat(this.level + 1, '"]'));
    ...
    onTabKey: function onTabKey() {
      var rows2 = _toConsumableArray$1(Y(this.$refs.node.parentElement, "tr"));
      var hasSelectedRow = rows2.some(function(row2) {
        return Q(row2, "data-p-selected") || row2.getAttribute("aria-checked") === "true";
      });
      ...
      if (hasSelectedRow) {
        var selectedNodes = rows2.filter(function(node) {
          return Q(node, "data-p-selected") || node.getAttribute("aria-checked") === "true";
        });
    ...
    setTabIndexForSelectionMode: function setTabIndexForSelectionMode(event2, nodeTouched) {
      if (this.selectionMode !== null) {
        var elements = _toConsumableArray$1(Y(this.$refs.node.parentElement, "tr"));
  ...
  methods: {
    ...
    sortNodesSingle: function sortNodesSingle(nodes) {
      ...
      return _toConsumableArray(nodes).sort(function(node1, node2) {
        var value1 = p(node1.data, _this.d_sortField);
        var value2 = p(node2.data, _this.d_sortField);
        return ee(value1, value2, _this.d_sortOrder, comparer);
      }).map(function(node) {
    ...
    multisortField: function multisortField(node1, node2, index) {
      var value1 = p(node1.data, this.d_multiSortMeta[index].field);
      var value2 = p(node2.data, this.d_multiSortMeta[index].field);
      var comparer = W();
      if (value1 === value2) {
        return this.d_multiSortMeta.length - 1 > index ? this.multisortField(node1, node2, index + 1) : 0;
      }
      return ee(value1, value2, this.d_multiSortMeta[index].order, comparer);
    },
    ...
    isFilterMatched: function isFilterMatched(node, _ref) {
      ...
      var matched = false;
      var dataFieldValue = p(node.data, filterField);
    ...
    onColumnKeyDown: function onColumnKeyDown(event2, col) {
      if ((event2.code === "Enter" || event2.code === "NumpadEnter") && event2.currentTarget.nodeName === "TH" && Q(event2.currentTarget, "data-p-sortable-column")) {
